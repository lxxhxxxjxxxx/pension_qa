...
- `app/retriever.py` — 문서 검색(BM25 계열 토큰 스코어 — IDF·TF 포화·길이 정규화)
...
- 테스트 커버리지 얕음
- 검색이 어휘 기반(BM25) — 임베딩·형태소 분석 아님. 조사가 붙은 토큰(`한도가` vs `한도`)은 여전히 안 겹침
- 근거 게이트 임계값(ADR 0002)이 현재 `data/` 3개 문서 실측 기준 — `tests/test_calibration.py`가 회귀를 잡음
