> 🎬 **촬영 시작점 `ch4-27-debt` — "몇 스프린트 굴린 pension_qa"를 재현해 둔 브랜치입니다.**  
> `ch4-27-done` + 부채 5건(TODO 4·FIXME 1)을 심었습니다. 코드 동작은 그대로고 테스트도 초록입니다(100 passed).
> 체인(`ch4-27-done`)은 깨끗하게 두고 부채는 여기에만 둡니다 — `ch2-14-buggy` 와 같은 방식.
> ⚠️ "TODO 합이 0"을 단언하는 테스트를 두면 이 브랜치에서 빨개져 21강 Stop 게이트가 영구 실패합니다. 그래서 두지 않았습니다.
