> 🎬 **29강 촬영 시작점 `ch4-29-stale`** — 28강을 마친 레포(`ch4-29-start`)에 자가점검·프로브·KPI 도구만 얹은 상태. **아직 아무 결정도 안 했다.**
> `python3 scripts/harness_audit.py --otel-log scripts/mock_otel_console.log` 를 치면 이 레포가 10주 만에 어디가 어긋났는지 나온다(게이트 항상 경고 · CLAUDE.md 거짓말 줄 · `/adr` 0회). 결정을 반영한 끝점은 `ch4-29-done`.
