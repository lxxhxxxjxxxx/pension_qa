...
# 주민등록번호 / 계좌·카드번호 비슷한 패턴 — 발견 시 "차단"
_PII_PATTERNS = [
    re.compile(r"\d{6}[- ]?\d{7}"),          # 주민등록번호
    re.compile(r"\d{3,4}[- ]\d{2,6}[- ]\d{3,6}"),  # 계좌/카드
]

# 이메일은 차단이 아니라 "마스킹"(ADR 0001) — 조치가 달라 _PII_PATTERNS와 분리해 둔다.
...
def mask_emails(text: str) -> str:
    """이메일을 `h***@e***.com` 꼴로 부분 마스킹한다(ADR 0001·SPEC.md).

    입력 경로(외부 LLM 전송 전)와 출력 경로 양쪽에서 쓴다.
    정규식 휴리스틱이라 오탐 가능 — 가드레일 규칙대로 fail-closed(과하게 마스킹) 방향.
    """
