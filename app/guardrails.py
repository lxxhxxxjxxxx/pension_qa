...
# 전문가 확인 권고 문구(키워드 휴리스틱 stub) — 하나라도 있으면 통과.
# 문구의 유무만 보며 답변의 공제·한도·수령 내용에는 관여하지 않는다.
...
def check_output_disclaimer(answer: str) -> Check:
    ...
    빈 답변도 문구가 없으므로 차단한다. 아직 `agent.py` 파이프라인에는
    연결되어 있지 않다(연결은 SYSTEM 프롬프트 수정과 함께 별도 작업).
    """
