...
# 전문가 확인 권고 문구 키워드(출력 disclaimer 판정 휴리스틱 — stub, 21강)
...
def check_output_disclaimer(answer: str) -> Check:
    """답변에 전문가 확인 권고 문구가 없으면 차단(fail-closed). 문구 유무만 본다 —
    공제·한도·수령 안내의 내용(금융 판단)은 건드리지 않는다. 빈 답변도 문구가 없으므로 차단."""
    if any(kw in answer for kw in _DISCLAIMER_KEYWORDS):
        return Check(True)
    return Check(
        False,
        "답변에 전문가 확인 권고 문구(예: '전문가와 상담하세요')가 없어 차단합니다.",
    )
